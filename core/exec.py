...
class Execution:
    ...
    def train(self, dataset, dataset_eval=None):
        ...
        for epoch in range(start_epoch, self.__C.MAX_EPOCH):
            ...
            # Iteration
            for step, (img_feat_iter,ques_ix_iter,ans_iter,size) in enumerate(dataloader): # added 'size' here
                ...
                for accu_step in range(self.__C.GRAD_ACCU_STEPS):
                    ...
                    pred = net(sub_img_feat_iter,sub_ques_ix_iter,size) # added 'size' here
                    if self.__C.MODEL=='mca':
    ...
    def eval(self, dataset, state_dict=None, valid=False):
        ...
        if self.plot_attention == True:
            # create list for Pi, Mu and Sigma
            Pi_list = []
            Mu_list=[]
            Sigma_list=[]
            all_sizes_list=[]
        ...
        for step, (
            ...
            if self.plot_attention == True:
                pred, Pi, Mu, Sigma, all_sizes = net(
                    img_feat_iter,
            ...
            # SAVE PI, MU AND SIGMA FOR PLOTS
            if self.plot_attention == True:
                Pi_np = Pi.cpu().data.numpy()
                Mu_np=Mu.cpu().data.numpy()
                Sigma_np=Sigma.cpu().data.numpy()
                all_sizes_np=all_sizes.cpu().data.numpy()
                for i in range (np.shape(Mu_np)[0]):
                    Pi_list.append(Pi_np[i].reshape(-1,))
                    Mu_list.append(Mu_np[i].reshape(-1,))
                    Sigma_list.append(Sigma_np[i].reshape(-1,))
                    all_sizes_list.append(all_sizes_np[i].reshape(-1,))
                ...
                if step==100:
                    if self.__C.CKPT_PATH is not None:
                        ...
                        ensemble_file = \
                            self.__C.PRED_PATH + \
                            'result_run_' + self.__C.CKPT_VERSION + \
                            '_epoch' + str(self.__C.CKPT_EPOCH) + '_100' \
                            '.json'

                    result_plot = [{
                        'pi': Pi_list[qix],
                        'mu': Mu_list[qix],
                        'sigma': Sigma_list[qix],
                        'size': all_sizes_list[qix],
                        'question_id': int(qid_list[qix])
                    }for qix in range(800)]          
                    pickle.dump(result_plot, open(ensemble_file, 'wb+'), protocol=-1)
